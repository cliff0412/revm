...
use primitives::{keccak256, Address, Bytes, StorageKey, StorageValue, B256, KECCAK_EMPTY, U256};
...
type StorageSender = OneshotSender<DatabaseResult<StorageValue>>;
...
type StorageFuture<Err> =
    Pin<Box<dyn Future<Output = (Result<StorageValue, Err>, Address, StorageKey)> + Send>>;
...
pub enum BackendFetchRequest {
    ...
    Storage(Address, StorageKey, StorageSender),
    /// Fetch a block hash
...
pub struct GlobalBackend {
    ...
    storage_requests: HashMap<(Address, StorageKey), Vec<StorageSender>>,
    ...
    fn on_request(&mut self, req: BackendFetchRequest) {
        match req {
            BackendFetchRequest::Basic(addr, sender) => {
                let acc = self.db.cache.accounts.get(&addr);
                if let Some(acc) = acc {
            ...
            BackendFetchRequest::BlockHash(number, sender) => {
                let hash = self.db.cache.block_hashes.get(&U256::from(number));
        ...
        match self.account_requests.entry(address) {
            ...
            Entry::Vacant(entry) => {
                ...
                let fut = Box::pin(async move {
                    ...
                    let balance = provider.get_balance(address).block_id(block_id);
                    let nonce = provider.get_transaction_count(address).block_id(block_id);
                    let code = provider.get_code_at(address).block_id(block_id);
            ...
            Entry::Vacant(entry) => {
                ...
                let fut = Box::pin(async move {
                    ...
                    let storage = provider
                        ...
                        .await;
                    // println!(
                    //     "fetch storage address: {:?}, slot: {:?}, val: {:?}",
                    //     address_ethers,
                    //     idx_ethers,
                ...
                match &mut request {
                    FetchRequestFuture::Basic(fut) => {
                        if let Poll::Ready((resp, addr)) = fut.poll_unpin(cx) {
                            // println!("fetch account addr: {:?}", addr,);
                            // get the response
                            ...
                            let (code, code_hash) = if !code.is_empty() {
                                // println!(
                                //     "fetch account res: code: {:?}, \n hash: {:?}",
                                //     hex::encode(code.clone()),
                                //     keccak256(&code)
                                // );
                    ...
                    FetchRequestFuture::Storage(fut) => {
                        if let Poll::Ready((resp, addr, idx)) = fut.poll_unpin(cx) {
                            // println!("fetch storage {:?} {:?}", addr, idx);
                    ...
                    FetchRequestFuture::BlockHash(fut) => {
                        if let Poll::Ready((block_hash, number)) = fut.poll_unpin(cx) {
                            ...
                            // update the cache
                            pin.db.cache.block_hashes.insert(U256::from(number), value);
